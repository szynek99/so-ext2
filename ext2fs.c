...
int ext2_block_used(uint32_t blkaddr) {
  ...
  uint32_t blkdata_idx = (blkaddr - 1) % blocks_per_group;
  uint32_t byte_idx = blkdata_idx / 8;
  uint32_t bit_idx = blkdata_idx % 8;

  blk_t *block_bitmap = blk_get(0, group_desc[group_id].gd_b_bitmap);
  used =
    ((*(char *)(block_bitmap->b_data + byte_idx)) & (1 << bit_idx)) >> bit_idx;
...
int ext2_inode_used(uint32_t ino) {
  ...
  blk_t *inode_bitmap = blk_get(0, group_desc[group_id].gd_i_bitmap);
  used =
    ((*(char *)(inode_bitmap->b_data + byte_idx)) & (1 << bit_idx)) >> bit_idx;
...
static int ext2_inode_read(off_t ino, ext2_inode_t *inode) {
  ...
  if (ext2_inode_used(ino)) {
    ...
    uint32_t idx = (ino - 1) % inodes_per_group;
    size_t pos =
      group_desc[group_id].gd_i_tables * BLKSIZE + idx * sizeof(ext2_inode_t);
...
static uint32_t ext2_blkptr_read(uint32_t blkaddr, uint32_t blkidx) {
  ...
  uint32_t pt = *((uint32_t *)(blk->b_data) + blkidx);
  blk_put(blk);
  return pt;
}
...
long ext2_blkaddr_read(uint32_t ino, uint32_t blkidx) {
  ...
  blkidx -= EXT2_NDADDR;
  uint32_t pt_in_block = BLKSIZE / sizeof(uint32_t);
  uint32_t curr_level;
  uint32_t max_idx = pt_in_block;
  if (blkidx < max_idx) {
    curr_level = ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR], blkidx);
    return curr_level;
  }
  blkidx -= max_idx;
  max_idx *= pt_in_block;
  if (blkidx < max_idx) {
    curr_level =
      ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR + 1], blkidx / pt_in_block);
    curr_level = ext2_blkptr_read(curr_level, blkidx % pt_in_block);
    return curr_level;
  }
  blkidx -= max_idx;
  max_idx *= pt_in_block;
  if (blkidx < max_idx) {
    curr_level = ext2_blkptr_read(inode.i_blocks[EXT2_NDADDR + 2],
                                  blkidx / (pt_in_block * pt_in_block));
    curr_level =
      ext2_blkptr_read(curr_level, (blkidx / pt_in_block) % pt_in_block);
    curr_level = ext2_blkptr_read(curr_level, blkidx % pt_in_block);
    return curr_level;
  }
  return -1;
...
int ext2_read(uint32_t ino, void *data, size_t pos, size_t len) {
  ...
  if (ino != 0) {
    ...
    if (inode.i_size < read_end)
      return EINVAL;
  }
  while (len > 0) {
    uint32_t blkaddr = pos / BLKSIZE;
    uint32_t idx_block = pos % BLKSIZE;
    uint32_t read_from_block =
      (len < BLKSIZE - idx_block) ? len : (BLKSIZE - idx_block);
    blk_t *blk = blk_get(ino, blkaddr);
    if (blk != BLK_ZERO) {
      memcpy(data, blk->b_data + idx_block, read_from_block);
      blk_put(blk);
    } else {
      memset(data, 0, read_from_block);
    }
    data += read_from_block;
    len -= read_from_block;
    pos += read_from_block;
  }
  return 0;
...
int ext2_readdir(uint32_t ino, uint32_t *off_p, ext2_dirent_t *de) {
  ...
  if (*off_p >= inode.i_size)
    return 0;
  while (ext2_read(ino, de, *off_p, de_name_offset) == 0) {
    if (de->de_ino != 0) {
...
int ext2_readlink(uint32_t ino, char *buf, size_t buflen) {
  ...
  if (EXT2_IFLNK == (EXT2_IFMT & inode.i_mode)) {
...
int ext2_stat(uint32_t ino, struct stat *st) {
  ...
  st->st_size = inode.i_size + inode.i_size_high;
